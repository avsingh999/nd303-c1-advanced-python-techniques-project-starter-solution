...
class Query(object):
    ...
    def __init__(self, **kwargs):
        """
        :param kwargs: dict of search query parameters to
        determine which SearchOperation query to use
        """
    ...
    def build_query(self):
        ...
        query = Query.Selectors(
                                ...
                                self.number,
                                self.filters,
                                Query.ReturnObjects[self.return_object]
                            )
...
class Filter(object):
    """
    Object representing optional filter options to be used in
    the date search for Near Earth Objects. Each filter is one
    of Filter.Operators provided with a field to filter on a value.
    """
    Options = {
    ...
    def create_filter_options(filter_options):
        ...
        defaultdict = {"NEO": [], "ORB": []}
        NeoFilter = dict()
        if filter_options is None or filter_options == []:
            return defaultdict
    ...
    def apply(self, results):
        ...
        if self.field == "diameter":
            filtered_list = list(filter(
                lambda neo: (
                                Filter.Operators[self.operation](
                                    getattr(neo, Filter.Options[self.field]),
                                    float(self.value)
                                )
                            ),
                results
            ...
            for orbit in all_orbits:
                ...
                if date_name not in unique_orbits:
                    if (Filter.Operators[self.operation](
                            orbit.miss_distance_kilometers,
                            float(self.value)
                            )):
...
class NEOSearcher(object):
    ...
    exposed by a generic  search interface get_objects, which, based
    on the query specifications, determines how to perform the search.
    """

    def __init__(self, db):
        """
        :param db: NEODatabase holding the NearEarthObject
        instances and their OrbitPath instances
        """
        self.db = db
        self.searchObject = []
    ...
    def equals_search(self, date_search):
        """
        Instance method to perform DateSearch on the db
        to extract NearEarthObject from a single date.

        Once the extraction is complete,
        return a list of the NearEarthObject obtained
    ...
    def between_search(self, date_search):
        """
        Instance method to perform DateSearch on the db
        to extract NearEarthObject from a a range of dates.
    ...
    def get_objects(self, query):
        ...
        """
        filters = (
                    Filter.create_filter_options(query.filters)
                    if query.filters is not None else None
                )
